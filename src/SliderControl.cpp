...
bool Slider_startHoming() {
    ...
    stepper.setAcceleration(HOMING_SPEED_STEPS_S * 4);
...
bool Slider_manualStart(Direction d) {
    ...
    // Speed is governed by maxSpeed + accel profile.
    state = STATE_MANUAL;
    return true;
}
...
void Slider_stopAuto() {
    if (state == STATE_AUTO_REPOSITION || state == STATE_AUTO_MOVE) {
        stepper.stop();
        state = STATE_IDLE;
    }
}
...
float Slider_autoProgress() {
    ...
    if (state != STATE_AUTO_MOVE)       return 0.0f;
